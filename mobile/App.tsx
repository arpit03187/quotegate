      ...
      <SafeAreaView style={styles.safe}>
        <ScrollView contentContainerStyle={styles.pad}>
          ...
          <Pressable style={styles.approve} disabled={busy} onPress={() => decide("approve")}>
            <Text style={styles.approveText}>{busy ? "Sending…" : "Swipe-right · Approve & send"}</Text>
          </Pressable>
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <ScrollView contentContainerStyle={styles.pad}>
        ...
        {items.length === 0 ? (
          <Text style={styles.sub}>No pending quotes. Pull to refresh after ingest.</Text>
        ) : null}
        {items.map((item) => (
          <Pressable key={item.proposal_id} style={styles.card} onPress={() => setActive(item)}>
